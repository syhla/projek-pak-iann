    ...
    <div class="border mt-6 p-4 rounded-lg text-[#556B2F] font-semibold">
      ...
      <ul class="mt-4 space-y-2">
        @foreach($categories as $categoryItem)
          <li>
            <a href="{{ route('category.show', ['category' => $categoryItem->id]) }}"
  ...
  <section class="mb-8">
    ...
    <div class="grid grid-cols-4 gap-6 mb-8">
      @foreach($newProducts as $produk)
        <div class="border rounded-lg bg-white shadow-md overflow-hidden relative">
          ...
          <div class="p-4 text-[#556B2F]">
              <h3 class="font-bold text-lg">{{ $produk->title }}</h3>
      ...
      <div class="mt-2 flex items-center text-yellow-500">
        ★★★★★
              ...
              {{-- Form Add to Cart --}}
<form action="{{ route('customer.cart.add', $produk->id) }}" method="POST">
    @csrf
    <button type="submit"
        class="mt-3 w-full bg-[#D4AF37] text-white py-1 rounded
               {{ $produk->stock == 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
